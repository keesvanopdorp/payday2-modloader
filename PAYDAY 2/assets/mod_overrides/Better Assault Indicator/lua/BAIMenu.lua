--[[
    ...
    BAI Menu specific code made by me
    List:
...
BAIMenu = BAIMenu or class()
function BAIMenu:init()
    ...
    self._tooltip_bottom_enabled = self._panel:text({
        ...
        font_size = 18,
        font = tweak_data.menu.pd2_medium_font,
        text = managers.localization:text("bai_enabled") .. ":",
        y = 10,
        w = 500,
        align = "right",
        wrap = true,
    ...
    self._tooltip_bottom_disabled = self._panel:text({
        ...
        font_size = 18,
        font = tweak_data.menu.pd2_medium_font,
        text = managers.localization:text("bai_disabled"),
        y = 10,
        w = 500,
        align = "right",
        wrap = true,
    ...
    local options_bg = self._panel:bitmap({
        name = "options_bg",
        texture = "guis/textures/pd2_mod_bai/menu_background",
        w = self._panel:w() / 2.5,
        h = self._panel:h(),
    })
    ...
    self:GetMenuFromJson(BAI.MenuPath .. "menu.json", BAI.settings)
    self:GetMenuFromJson(BAI.MenuPath .. "assault_box.json", BAI.settings.assault_panel.assault)
    self:GetMenuFromJson(BAI.MenuPath .. "captain_box.json", BAI.settings.assault_panel.captain)
    self:GetMenuFromJson(BAI.MenuPath .. "endless_box.json", BAI.settings.assault_panel.endless)
    self:GetMenuFromJson(BAI.MenuPath .. "survived_box.json", BAI.settings.assault_panel.survived)
    self:GetMenuFromJson(BAI.MenuPath .. "escape_box.json", BAI.settings.assault_panel.escape)
    self:GetMenuFromJson(BAI.MenuPath .. "assault_states.json")
    self:GetMenuFromJson(BAI.MenuPath .. "advanced_assault_info.json", BAI.settings.advanced_assault_info)
    self:GetMenuFromJson(BAI.MenuPath .. "animations.json", BAI.settings.animation)
    self:GetMenuFromJson(BAI.MenuCompatibilityPath .. "holoui.json", BAI.settings.hud.holoui)
    self:GetMenuFromJson(BAI.MenuCompatibilityPath .. "pdth_hud_reborn.json", BAI.settings.hud.pdth_hud_reborn)
    self:GetMenuFromJson(BAI.MenuCompatibilityPath .. "restoration_mod.json", BAI.settings.hud.restoration_mod)
    self:GetMenuFromJson(BAI.MenuCompatibilityPath .. "restoration_mod/assault_box.json")
    self:GetMenuFromJson(BAI.MenuCompatibilityPath .. "restoration_mod/endless_box.json")
    self:GetMenuFromJson(BAI.MenuCompatibilityPath .. "restoration_mod/survived_box.json")
    self:GetMenuFromJson(BAI.MenuCompatibilityPath .. "restoration_mod/assault_states.json")
    self:GetMenuFromJson(BAI.MenuCompatibilityPath .. "halo_reach_hud.json", BAI.settings.hud.halo_reach_hud)
    --MenuHelper:LoadFromJsonFile(BAI.MenuPath .. "modules.json")

    -- Custom Text for Normal and Endless Assaults
    local custom_text = BAI.MenuPath .. "custom_text/"
    local difficulty = custom_text .. "difficulty/"
    local faction = custom_text .. "faction/"

    local ct =
    {
        ["long"] = "custom_text",
        ["short"] = "short_custom_text"
    }
    local boxes =
    {
        "assault",
    ...
    for _, v in pairs(boxes) do
        for i, j in pairs(ct) do
            self:GetMenuFromJson(difficulty .. v .. "_box_" .. i .. ".json", BAI.settings.assault_panel[v][j])
            self:GetMenuFromJson(faction .. v .. "_box_" .. i .. ".json", BAI.settings.assault_panel[v][j])
        end
    end

    self:OpenMenu("bai_menu")

    self:InitAssaultPanel()
    self:InitTextures()
end

function BAIMenu:CallCallback(item, params)
...
function BAIMenu:SetAxisTimer(axis, delay, input_delay, input)
...
function BAIMenu:Open()
    ...
    if not self._controller then
        self._controller = managers.controller:create_controller("BAIMenu", nil, false)
...
-- Mouse Functions
function BAIMenu:mouse_move(o, x, y)
...
function BAIMenu:mouse_press(o, button, x, y)
    x, y = managers.mouse_pointer:convert_fullscreen_16_9_mouse_pos(x, y)
...
function BAIMenu:mouse_release(o, button, x, y)
    x, y = managers.mouse_pointer:convert_fullscreen_16_9_mouse_pos(x, y)
    if button == Idstring("0") then	
        if self._slider then
...
function BAIMenu:MenuLeftRight(change)
    if not self._enabled then
        return
    end
...
function BAIMenu:ActivateItem(item, x)
...
function BAIMenu:SetMenuItemsEnabled(menu, parent_item_id, enabled)
...
function BAIMenu:GetMenu(menu)
    return self._menus[menu]
end

function BAIMenu:AnimateItemEnabled(item, enabled)
...
function BAIMenu:HighlightItem(item)
...
function BAIMenu:UnhighlightItem(item)
    if item.focus_changed_callback then
...
function BAIMenu:SetLegends(accept, reset, step)
...
function BAIMenu:SetItem(item, value, menu)
...
function BAIMenu:GetMenuFromJson(path, values)
    ...
    if file then
        ...
        if content.title == "bai_mod_title" then
            menu_title = menu_title .. " r" .. BAI.ModVersion
        end
        ...
        for i, item in pairs(items) do
            ...
            if item.special_remarks then
                value = BAI.settings.assault_panel[item.special_remarks][item_type == "toggle" and "enabled" or "custom_text"]
            end
            ...
            elseif parents then
                if type(parents) == "string" then
                    for _, pitem in pairs(items) do
                        if pitem.id == parents then
                            enabled = BAI.settings.assault_panel[pitem.special_remarks].enabled
                            break
                        end
                    end
                end
...
function BAIMenu:CreateMenu(params)
...
function BAIMenu:OpenMenu(menu, close)
...
function BAIMenu:AddItemToMenu(menu_id, item)
...
function BAIMenu:GetLastPosInMenu(menu_id)
    return self._menus[menu_id].len or 50
end

--Label Items
function BAIMenu:CreateLabel(params)
...
--Divider Items
function BAIMenu:CreateDivider(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
...
--Button Items
function BAIMenu:CreateButton(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
...
--Toggle Items
function BAIMenu:CreateToggle(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
...
--Slider Items
function BAIMenu:CreateSlider(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
...
function BAIMenu:SetSlider(item, x, add)
...
function BAIMenu:SetInput(item)
    if self._input then
        self:CloseInput()
    end
    self._input = item
...
function BAIMenu:key_press(item, o, k)
...
function BAIMenu:enter_text(item, o, s)
    local text = item.value
    local m = item.limit
    local n = utf8.len(text)
    s = utf8.sub(s, 1, m - n)
...
function BAIMenu:key_release(item, o, k)
    if BAI:IsOr(k, Idstring("esc"), Idstring("enter")) then
        self:CloseInput()
    end
end

function BAIMenu:CloseInput()
    if self._input then
...
function BAIMenu:CreateMultipleChoice(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
...
function BAIMenu:OpenMultipleChoicePanel(item)
...
function BAIMenu:CloseMultipleChoicePanel()
...
function BAIMenu:CreateInput(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
...
-- Custom Color Items
function BAIMenu:CreateColorSelect(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
...
function BAIMenu:OpenColorMenu(item)
...
function BAIMenu:SetColorSlider(item, x, type, add)
...
function BAIMenu:CloseColorMenu()
...
function BAIMenu:ResetColorMenu()
...
function BAIMenu:InitAssaultPanel()
end

function BAIMenu:InitTextures()
    local right = self._tooltip:right()
    local panel = self._panel
    local textures =
    {
        "difficulty_name_enabled",
        "difficulty_name_disabled",
        "aai_panel_1",
        "aai_panel_2",
        "captain_panel_enabled",
        "captain_panel_disabled"
    }
    for _, v in pairs(textures) do
        self["_menu_" .. v] = panel:bitmap({
            texture = "guis/textures/pd2_mod_bai/menu/" .. v,
            valign = 'center',
            halign = 'center',
            visible = false,
            layer = 500,
        ...
        self["_menu_" .. v]:set_right(right)
    end
end

dofile(BAI.LuaPath .. "BAIMenuCallbacks.lua")