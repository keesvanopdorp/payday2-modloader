...
function BAIAnimation:AAIPanel(p, bg)
    local bg = managers.hud._hud_assault_corner[bg]:child("bg")
...
function BAIAnimation:AAIPanelVoidUI(p, blink)
    self:FadeIn(p)
    p:child("bg"):animate(blink)
end

function BAIAnimation:AAIPanelHoloUI(p, bg, child)
    local bg = managers.hud._hud_assault_corner[bg]
    bg:child("bg"):stop()
    local child = bg:child(child)

    self:FadeIn(p)

    child:stop()
    play_value(child, "alpha", 0.25, {time = 1, callback = function()
        play_value(child, "alpha", 1, {time = 1})
    end})
end
